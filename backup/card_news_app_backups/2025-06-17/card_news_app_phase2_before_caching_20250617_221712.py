...
# Components import (set_page_config 이후에 위치해야 함)
...
def render_summary_tab():
    ...
    # 원본 CSS 파일 읽기
    css_path = get_path('output_html').parent / 'templates' / 'original_summary_style.css'
    if css_path.exists():
...
"""
    ...
    for i, card in enumerate(card_news_list):
        # 파일 경로 수정 (Base64 Data URL 사용)
        # 파일명만 추출하고 절대 경로로 변환
        filename = Path(card["file_path"]).name
        actual_path = get_path('output_html') / filename
        ...
        card_html = f'''
            ...
            <div class="news-card" onclick="window.open('{file_path}', '_blank')">
            ...
            <a href="{file_path}" class="read-more" onclick="event.stopPropagation()">자세히 보기 →</a>
            </div>
            </div>
'''
        ...
        with open(get_path_str('summary_json'), 'r', encoding='utf-8') as f:
            ...
            for card in cards:
                ...
                card_news_list.append({
                    ...
                    "file_path": card.get('file_path', ''),  # 파일명만 저장
        ...
        for html_file in html_dir.glob("detail_*.html"):
            ...
            if len(parts) == 2:
                ...
                card_news_list.append({
                    ...
                    "category": "general",
                    "category_name": "일반",
                    "file_path": html_file.name,  # 파일명만 저장
