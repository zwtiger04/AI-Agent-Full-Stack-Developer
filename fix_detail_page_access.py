"""
상세 페이지 접근 문제 해결 스크립트
"""
import os
from pathlib import Path

def create_symlink_for_static_files():
    """Streamlit static 폴더에 심볼릭 링크 생성"""
    # Streamlit의 static 폴더 찾기
    streamlit_static = Path.home() / ".streamlit" / "static"
    streamlit_static.mkdir(parents=True, exist_ok=True)
    
    # 카드뉴스 HTML 폴더
    cardnews_html = Path("output/card_news/html").absolute()
    
    # 심볼릭 링크 생성
    link_path = streamlit_static / "cardnews"
    if link_path.exists():
        link_path.unlink()
    
    try:
        link_path.symlink_to(cardnews_html)
        print(f"✅ 심볼릭 링크 생성: {link_path} -> {cardnews_html}")
        return True
    except Exception as e:
        print(f"❌ 심볼릭 링크 생성 실패: {e}")
        return False

def update_summary_ui_with_proper_links():
    """summary_ui.py의 링크 방식 수정"""
    content = '''"""
...
'''
    ...
    # 파일 저장
    with open("summary_ui_fixed.py", "w", encoding="utf-8") as f:
        f.write(content)
    
    print("✅ summary_ui_fixed.py 생성 완료")

if __name__ == "__main__":
    update_summary_ui_with_proper_links()
